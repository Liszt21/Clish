...
(defun get-profile-path (&optional (name "clish"))
  (concatenate 'string "~/" #+os-windows "Documents/WindowsPowerShell/"
                            #+os-windows (format nil "~A.ps1" (string-capitalize name))
                            #-os-windows (format nil ".~Arc" name)))
...
(defmacro with-profile (options &body body)
  (destructuring-bind (ref &key section (module "clish") (remove-if-empty t)) options
    (let* ((path (get-profile-path module))
